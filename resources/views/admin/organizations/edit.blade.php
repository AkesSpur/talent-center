<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="min-w-0">
                <h2 class="font-serif text-xl sm:text-2xl font-bold text-dark">Редактирование организации</h2>
                <p class="text-warm-gray mt-1 truncate">{{ $organization->name }}</p>
            </div>
            <a href="{{ route('admin.organizations.show', $organization) }}" class="self-start sm:self-auto px-4 py-2 text-warm-gray hover:text-primary transition-colors text-sm shrink-0">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="POST" action="{{ route('admin.organizations.update', $organization) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    <h3 class="font-serif text-xl font-semibold text-dark mb-4">Данные организации</h3>

                    {{-- Avatar --}}
                    <div class="mb-6 pb-6 border-b border-primary/10" x-data="{ preview: null, hasAvatar: {{ $organization->avatar_path ? 'true' : 'false' }} }">
                        ...
                        <div class="flex items-center gap-5">
                            ...
                            <div class="space-y-2">
                                <label for="admin-org-avatar-input" class="inline-flex items-center gap-2 px-4 py-2 border border-primary/30 text-primary text-sm font-medium rounded-lg hover:bg-primary/5 cursor-pointer transition-colors">
                                    <i class="fas fa-camera"></i> Изменить логотип
                                </label>
                                <input id="admin-org-avatar-input" name="avatar" type="file" accept="image/*" class="hidden"
                        ...
                        @error('avatar') <p class="text-red-600 text-xs mt-2">{{ $message }}</p> @enderror
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            <label for="name" class="block text-sm font-medium text-dark mb-2">Название <span class="text-red-500">*</span></label>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="description" name="description" rows="3"
                                class="w-full px-4 py-3 border border-primary/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary resize-none">{{ old('description', $organization->description) }}</textarea>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            @error('ogrn') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                        ...
                        <div>
                            <label for="contact_phone" class="block text-sm font-medium text-dark mb-2">Телефон</label>
