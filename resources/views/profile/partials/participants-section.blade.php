<div x-data="participantsManager()" class="space-y-6">
                ...
                @foreach($participants as $participant)
                    <div class="flex items-center gap-4 p-4 border border-primary/10 rounded-lg hover:bg-cream/50 transition-colors">
                        ...
                        <div class="flex gap-2 flex-shrink-0">
                            <button
                                @click="openEditModal({{ $participant->id }}, '{{ addslashes($participant->last_name) }}', '{{ addslashes($participant->first_name) }}', '{{ addslashes($participant->patronymic ?? '') }}')"
    ...
    <div x-show="showModal" x-cloak
        ...
        <div @click.outside="closeModal()" class="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto"
            x-transition:enter="ease-out duration-300"
            ...
            <form :action="editingId ? '{{ url('participants') }}/' + editingId : '{{ route('participants.store') }}'" method="POST" class="p-6 space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                        <label class="block text-sm font-medium text-dark mb-1">Фамилия <span class="text-red-500">*</span></label>
                        <input name="last_name" type="text" required x-model="form.last_name"
                            class="w-full px-3 py-2 border border-primary/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-dark mb-1">Имя <span class="text-red-500">*</span></label>
                ...
                <div class="flex gap-3 pt-4">
                    ...
                    <button type="submit"
                        class="flex-1 py-2 gradient-gold text-dark font-medium rounded-lg hover:opacity-90 transition-opacity">
                        Сохранить
                    </button>
                </div>
            </form>
...
function participantsManager() {
    return {
        ...
        openAddModal() {
            this.editingId = null;
            this.form = { last_name: '', first_name: '', patronymic: '' };
            this.showModal = true;
        },
        openEditModal(id, lastName, firstName, patronymic) {
            this.editingId = id;
            this.form = { last_name: lastName, first_name: firstName, patronymic: patronymic };
            this.showModal = true;
        },
        closeModal() {
